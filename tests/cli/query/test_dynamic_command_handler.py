...
class TestDynamicCommandHandler(unittest.TestCase):
    ...
    def test_handle_file_operation(self, mock_update_metadata, mock_print_success, mock_print_info):
        ...
        self.executor.perform_file_operation.assert_called_once_with(
            'CREATE', os.path.join(self.initial_dir, 'test.txt'), 'Test content', force=True)
    ...
    @patch('drd.cli.query.dynamic_command_handler.print_success')
    @patch('drd.cli.query.dynamic_command_handler.update_file_metadata')
    def test_handle_file_operation_update(self, mock_update_metadata, mock_print_success, mock_print_info):
        cmd = {'operation': 'UPDATE', 'filename': 'test.txt',
        ...
        self.assertEqual(output, "Success")
        self.executor.perform_file_operation.assert_called_once_with(
            'UPDATE', os.path.join(self.initial_dir, 'test.txt'), 'Change content', force=True)
        mock_update_metadata.assert_called_once_with(
            cmd, self.metadata_manager, self.executor)
    ...
    def test_handle_cd_command(self, mock_abspath, mock_chdir):
        mock_abspath.return_value = os.path.join(self.initial_dir, 'app')
        result = self.executor._handle_cd_command('cd app')
        self.assertEqual(result, f"Changed directory to: {os.path.join(self.initial_dir, 'app')}")
        mock_chdir.assert_called_once_with(os.path.join(self.initial_dir, 'app'))
        self.assertEqual(self.executor.current_dir, os.path.join(self.initial_dir, 'app'))
    ...
    def test_perform_file_operation_update(self, mock_preview, mock_confirm, mock_file, mock_exists):
        ...
        self.assertTrue(result)
        mock_file.assert_any_call(os.path.join(self.initial_dir, 'test.txt'), 'r')
        mock_file.assert_any_call(os.path.join(self.initial_dir, 'test.txt'), 'w')
    ...
    def test_reset_directory(self, mock_chdir):
        self.executor.current_dir = os.path.join(self.initial_dir, 'app')
