...
class TestDynamicCommandHandler(unittest.TestCase):
    ...
    def test_update_file_metadata(self, mock_generate_description, mock_print_error):
        cmd = {'filename': 'test.txt', 'content': 'Test content'}
        mock_generate_description.return_value = (
            'python', 'Test file', ['test_function'], ['import os', 'import sys'], ['export1', 'export2'])
        ...
        self.metadata_manager.update_file_metadata.assert_called_once_with(
            'test.txt', 'python', 'Test content', 'Test file', ['test_function'], ['import os', 'import sys'], ['export1', 'export2'])
    ...
    def test_handle_error_with_dravid(self, mock_echo, mock_execute_commands,
        ...
        mock_print_info.assert_any_call(
            "Handling error with Dravid API. Attempt 1/3...", indent=2)
        mock_print_info.assert_any_call(
            "Sending error information to Dravid for analysis...", indent=2)
    ...
    def test_execute_commands_with_requires_restart(self, mock_print_debug, mock_print_info, mock_print_step):
        ...
        self.assertEqual(steps_completed, 2)
        self.assertIsNone(error)
        self.assertIn("Requires_restart command - Server needs to be restarted", output)
        self.assertIn("Shell command - echo \"Hello\"", output)
        mock_print_debug.assert_has_calls([
            call("Completed step 1/2"),
            call("Completed step 2/2")
