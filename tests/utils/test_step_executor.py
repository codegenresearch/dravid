...
class TestExecutor(unittest.TestCase):
    ...
    def test_perform_file_operation_create(self, mock_file, mock_exists):
        ...
        result = self.executor.perform_file_operation(
            'CREATE', os.path.join(self.initial_dir, 'test.txt'), 'content')
    ...
    def test_perform_file_operation_delete(self, mock_remove, mock_isfile, mock_exists):
        ...
        result = self.executor.perform_file_operation('DELETE', os.path.join(self.initial_dir, 'test.txt'))
        self.assertTrue(result)
        mock_remove.assert_called_with(os.path.join(
            self.initial_dir, 'test.txt'))
    ...
    def test_perform_file_operation_update(self, mock_preview, mock_confirm, mock_file, mock_exists):
        ...
        result = self.executor.perform_file_operation(
            'UPDATE', os.path.join(self.initial_dir, 'test.txt'), changes)
        ...
        mock_preview.assert_called_once_with(
            'UPDATE', os.path.join(self.initial_dir, 'test.txt'), new_content=expected_updated_content, original_content="original content")
    ...
    def test_perform_file_operation_user_cancel(self, mock_confirm):
        ...
        result = self.executor.perform_file_operation(
            'UPDATE', os.path.join(self.initial_dir, 'test.txt'), 'content')
