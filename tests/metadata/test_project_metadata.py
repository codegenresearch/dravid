...
class TestProjectMetadataManager(unittest.TestCase):
    ...
    async def test_build_metadata(self, mock_walk, mock_analyze_file):
        ...
        try:
            ...
            self.assertEqual(len(metadata['key_files']), 1, "Expected one key file in metadata")
    ...
    def test_get_ignore_patterns_file_error(self, mock_file):
        with self.assertRaises(OSError) as context:
            self.manager.get_ignore_patterns()
        self.assertEqual(str(context.exception), "Failed to open file", "Expected OSError with specific message")

    @patch('src.drd.metadata.project_metadata.call_dravid_api_with_pagination', side_effect=Exception("API call failed"))
    @patch('builtins.open', new_callable=mock_open, read_data='print("Hello, World!")')
    async def test_analyze_file_api_error(self, mock_file, mock_api_call):
        with self.assertRaises(Exception) as context:
            await self.manager.analyze_file('/fake/project/dir/script.py')
        self.assertEqual(str(context.exception), "API call failed", "Expected Exception with specific message")

    @patch('os.walk', side_effect=OSError("Failed to walk directory"))
    async def test_build_metadata_walk_error(self, mock_walk):
        loader = MagicMock()
        with self.assertRaises(OSError) as context:
            await self.manager.build_metadata(loader)
        self.assertEqual(str(context.exception), "Failed to walk directory", "Expected OSError with specific message")

    @patch('src.drd.metadata.project_metadata.ProjectMetadataManager.analyze_file', side_effect=Exception("Analyze file failed"))
    @patch('os.walk')
    async def test_build_metadata_analyze_error(self, mock_walk, mock_analyze_file):
        mock_walk.return_value = [
            ('/fake/project/dir', [], ['main.py', 'README.md'])
        ]
        loader = MagicMock()
        with self.assertRaises(Exception) as context:
            await self.manager.build_metadata(loader)
        self.assertEqual(str(context.exception), "Analyze file failed", "Expected Exception with specific message")