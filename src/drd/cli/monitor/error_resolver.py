...
def monitoring_handle_error_with_dravid(error, line, monitor):
    ...
    user_input = monitor.get_user_input(
        "🛠️ Do you want to proceed with this fix? Type 'y' to apply or 'n' to skip: "
    )
    ...
    if user_input.lower() == 'y':
        ...
        if requires_restart:
            ...
            restart_input = monitor.get_user_input(
                "🔁 Do you want to restart the server now? Type 'y' to restart or 'n' to skip: "
            )
        ...
        print_info("🛠️ Fix not applied. Continuing with current state.")
        return False