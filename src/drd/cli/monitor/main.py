...
def run_dev_server_with_monitoring(command: str):
    print_header("Initializing server monitor for the following command:")
    ...
    try:
        monitor.start()
        print_success("Server monitor is now active.")
        print_prompt("Press Ctrl+C to stop the server monitor.")
        while not monitor.should_stop.is_set():
            pass
        print_info("Server monitor has gracefully ended.")
...
def handle_syntax_error(error_msg, monitor):
    error = SyntaxError(f"Syntax error detected in your code: {error_msg}")
    print_error(f"Error detected: {error}")
    monitoring_handle_error_with_dravid(error, error_msg, monitor)


def handle_general_error(error_msg, monitor):
    error = Exception(f"A general error occurred: {error_msg}")
    print_error(f"Error detected: {error}")
    monitoring_handle_error_with_dravid(error, error_msg, monitor)