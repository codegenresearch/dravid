...
class OutputMonitor:
            ...
            if self.monitor.process.poll() is not None and not self.monitor.processing_input.is_set():
                if not self.monitor.restart_requested.is_set():
                    ...
                    if self.retry_count < MAX_RETRIES:
                        ...
                        print_error(f"Failed to restart the server after {MAX_RETRIES} attempts. Exiting the monitoring mode.")
                        self.monitor.stop()
                        break
                continue
    ...
    def _check_idle_state(self):
        ...
        if (time_since_last_output > 5 and
            ...
            print_info("\nNo further tasks are currently being processed automatically. How may I assist you next?")
            self._show_options()
            self.idle_prompt_shown = True

    def _show_options(self):
        print_info("\nAvailable actions:")
        print_info("1. Provide a coding instruction for execution")
        print_info("2. Process an image by typing 'vision'")
