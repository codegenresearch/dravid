...
2. Renaming the `start_process` function to `_start_process` to match the gold code's naming convention.
3. Ensuring that `_start_process` is called with the correct parameters, particularly `self.project_dir` for `cwd`.
4. Ensuring error handling in `_start_process` is consistent with the gold code by calling `self.stop()` in the event of an error.
