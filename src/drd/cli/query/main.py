...
def execute_dravid_command(query, image_path, debug, instruction_prompt, warn=None):
    ...
    if warn:
        ...
        print_warning(
            "If it's an existing project, please make sure you're on a separate git branch. 🏁")
    ...
    try:
        ...
        if project_context:
            ...
            print_info(
                "No current project context found. Creating a new project in the current directory... 📂")
            full_query = f"User query: {query}"

        print_info("Preparing to send query to LLM... 📡")
        if image_path:
            print_info(f"Processing image: {image_path} 🖼️")
            print_info("LLM calls to be made: 1 🔄")
        ...
        if not success:
            ...
            print_error(f"Error message: {error_message} ⚠️")
            ...
            if handle_error_with_dravid(Exception(error_message), commands[step_completed-1], executor, metadata_manager, debug=debug):
                ...
                print_error(
                    "Unable to fix the error. Skipping this command and continuing with the next. ⏭️")
