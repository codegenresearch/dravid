...
from ...utils import print_error, print_success, print_info, run_with_loader
...
def execute_dravid_command(query, image_path, debug, instruction_prompt):
    ...
    print_warning("Ensure you're in a fresh directory or a git branch. Use Ctrl+C to exit.")
    ...
    try:
        ...
        if image_path:
            ...
            commands = run_with_loader(
                lambda: call_dravid_vision_api(full_query, image_path, True, instruction_prompt),
                "Analyzing image and generating response"
            )
        else:
            print_info("Streaming response from Claude API...")
            xml_result = stream_dravid_api(full_query, True, instruction_prompt, False)
        ...
        success, step_completed, error_message, all_outputs = execute_commands(
            commands, executor, metadata_manager, debug)
        ...
        if not success:
            ...
            if handle_error_with_dravid(Exception(error_message), commands[step_completed-1], executor, metadata_manager, debug):
                ...
                success, _, error_message, additional_outputs = execute_commands(
                    remaining_commands, executor, metadata_manager, debug)
                all_outputs += "\n" + additional_outputs
            else:
