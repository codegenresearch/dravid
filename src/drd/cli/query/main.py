...
def construct_full_query(query, executor, project_context, files_info=None, reference_files=None):
    ...
    if is_empty:
        ...
        full_query = f"<projectContext>{project_context}</projectContext>\n"
    ...
    if reference_files:
        ...
        reference_context = "\n\n".join(
            [f"<referenceFile><file>{file}</file><content>{content}</content></referenceFile>" for file, content in reference_contents.items()])
...
2. **Consistent Dictionary Access**: Ensured that dictionary keys are accessed directly when certain keys are guaranteed to exist, and used `.get()` where necessary to avoid potential `KeyError`.
...
7. **Comments and Documentation**: Added comments where necessary to explain complex logic or important sections of the code.
