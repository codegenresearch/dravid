...
def execute_dravid_command(query, image_path, debug, instruction_prompt, warn=None):
    ...
    try:
        ...
        if project_context:
            print_info("Identifying files related to the query...")
        ...
        print_info(f"Parsed {len(commands)} commands from LLM's response.")
        ...
        if not success:
            ...
            if handle_error_with_dravid(Exception(error_message), commands[step_completed-1], executor, metadata_manager, debug=debug):
                ...
                print_error("Unable to fix the error. Skipping command and continuing with next.")
