...
def execute_dravid_command(query, image_path, debug, instruction_prompt, warn=None):
    ...
    if warn:
        print_warning("Ensure you are in a fresh directory or a git branch")
    ...
    try:
        ...
        if project_context:
            ...
            print_info(f"Found {len(files_to_modify)} relevant files")
            if debug:
            ...
            print_info("No project context found. Creating new project")
        ...
        if not commands:
            print_error("Failed to parse response or no commands to execute")
        ...
        if not success:
            print_error(f"Failed at step {step_completed}: {error_message}")
            print_info("Attempting to fix error")
            if handle_error_with_dravid(Exception(error_message), commands[step_completed-1], executor, metadata_manager, debug=debug):
                print_info("Fix applied. Continuing")
                ...
                print_error("Unable to fix error. Skipping")
