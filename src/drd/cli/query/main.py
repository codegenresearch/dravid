    ...
    try:
        ...
        if not success:
            ...
            if handle_error_with_dravid(Exception(error_message), commands[step_completed-1], executor, metadata_manager, debug=debug):
                ...
                print_error("🛠️   Unable to fix the error. Skipping this command and continuing with the next.")
...
1. Adding the necessary print statements with correct indentation and messages.
2. Enhancing readability with emojis and symbols.
3. Improving error messages for clarity.
4. Adding context about the directory status when no project context is found.
5. Ensuring consistent formatting and structure throughout the function.