...
def execute_dravid_command(query, image_path, debug, instruction_prompt, warn=None):
    print_info("🚀 Starting Dravid AI...")
    if warn:
        print_warning("⚠️ Please ensure you are in a fresh directory.")
        print_warning("⚠️ If it is an existing project, ensure you are on a git branch.")
    ...
    try:
        ...
        if project_context:
            ...
            full_query = (
                ...
                f"📁 Current directory is not empty.\n\n"
                f"🔎 User query: {query}"
            )
        else:
            is_empty = is_directory_empty(executor.current_dir)
            print_info(
                f"🔍 No current project context found. {'📁 Current directory is empty.' if is_empty else '📁 Will create a new project in the current directory.'}")
        ...
        if not success:
            ...
            if handle_error_with_dravid(Exception(error_message), commands[step_completed-1], executor, metadata_manager, debug=debug):
                print_info("  🛠️ Fix applied successfully. Continuing with the remaining commands.")
        ...
        print_success("🚀 Dravid AI execution completed.")
...
2. Ensuring consistent print statements and indentation levels.
3. Adjusting message clarity and wording to match the gold code.
4. Improving error messages for consistency.
5. Providing specific context about the directory status when no project context is found.
6. Ensuring consistent use of emojis and symbols.
7. Maintaining functionality consistency with the gold code.