    ...
    for i, cmd in enumerate(commands, 1):
        ...
        try:
            if cmd['type'] == 'explanation':
                ...
                output = 'Server restart required if running'
            else:
            ...
            if isinstance(output, str) and output.startswith("Skipping"):
                ...
                print_success(f"Step {i}/{total_steps}: {cmd['type'].capitalize()} command - {cmd.get('command', '')} {cmd.get('operation', '')}")
                if output:
...
def update_file_metadata(cmd, metadata_manager, executor):
    ...
    print_debug(f"Updated metadata for {cmd['filename']} with type: {file_type}, description: {description}")
...
def handle_error_with_dravid(error, cmd, executor, metadata_manager, depth=0, previous_context="", debug=False):
    ...
    print_info("Submitting error information to Dravid for analysis...", indent=2)
