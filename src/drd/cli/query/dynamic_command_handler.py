        ...
        if cmd['type'] == 'explanation':
            ...
            try:
                ...
                if isinstance(output, str) and output.startswith("Skipping"):
                    print_info(f"⚠️ Step {i}/{total_steps}: {output}")
...
def handle_error_with_dravid(error, cmd, executor, metadata_manager, depth=0, previous_context="", debug=False):
    ...
    if fix_applied:
        ...
        user_input = click.prompt("Would you like to try fixing the issue again? [y/N]", type=str, default='N')
        if user_input.lower() == 'y':
            ...
            print_info("💡 Fix application aborted.")
            return False