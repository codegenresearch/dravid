...
def handle_shell_command(cmd, executor):
    ...
    print_success(f"Successfully executed: {cmd['command']} ✔️")
    if output:
...
def handle_error_with_dravid(error, cmd, executor, metadata_manager, depth=0, previous_context="", debug=False):
    ...
    user_input = click.confirm("Do you want to proceed with this fix? You can stop at any time during the process. ✅", default=False)
    if user_input:
