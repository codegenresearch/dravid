    ...
    for index, cmd in enumerate(commands, start=1):
        ...
        if cmd_type == 'shell':
            ...
            if operation == 'UPDATE_DEV_SERVER':
                print_info(
                    f"  Start Command: {cmd.get('start_command', 'N/A')}", indent=6)
...
1. Using `Fore.MAGENTA` for the `print_prompt` function.
2. Including the `indent` parameter in `print_info` calls within `print_command_details`.
3. Simplifying and structuring the `create_confirmation_box` function.
4. Adjusting the `print_header` function to include a specific emoji and format it similarly.
5. Removing unused imports (`json` and `os`).