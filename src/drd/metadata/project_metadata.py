...
class ProjectMetadataManager:
    ...
    def load_metadata(self):
        if os.path.exists(self.metadata_file):
            try:
                ...
                print_error(f"Error decoding JSON from {self.metadata_file}: {str(e)}")
                return {}
            except Exception as e:
                print_error(f"Error reading {self.metadata_file}: {str(e)}")
                return {}
    ...
    def save_metadata(self):
        try:
            ...
            print_error(f"Error saving metadata to {self.metadata_file}: {str(e)}")
    ...
    def get_directory_structure(self, start_path):
        ...
        try:
            ...
            print_error(f"Error getting directory structure from {start_path}: {str(e)}")
        return structure
    ...
    async def analyze_file(self, file_path):
        ...
        try:
            ...
            file_info = {
                ...
                "exports": [dep.strip() for dep in metadata.find('exports').text.split(',') if dep.strip() != 'None'],
                "imports": [dep.strip() for dep in metadata.find('imports').text.split(',') if dep.strip() != 'None']
            }

            dependencies = metadata.find('external_dependencies')
            if dependencies is not None:
                for dep in dependencies.findall('dependency'):
                    dep_text = dep.text.strip()
                    if dep_text and dep_text not in self.metadata['external_dependencies']:
                        self.metadata['external_dependencies'].append(dep_text)

            return file_info

        except ET.ParseError as e:
            print_warning(f"Error parsing XML response for file {file_path}: {str(e)}")
        except Exception as e:
    ...
    async def build_metadata(self, loader):
        ...
        if all_languages:
            self.metadata['environment']['primary_language'] = max(
                all_languages, key=lambda x: sum(1 for file in self.metadata['key_files'] if file['type'] == x))
    ...
    def remove_file_metadata(self, filename):
        try:
            ...
            print_error(f"Error removing metadata for file {filename}: {str(e)}")
    ...
    def add_external_dependency(self, dependency):
        try:
            dep_text = dependency.strip()
            if dep_text and dep_text not in self.metadata['external_dependencies']:
                self.metadata['external_dependencies'].append(dep_text)
                self.save_metadata()
        except Exception as e:
            print_error(f"Error adding external dependency {dependency}: {str(e)}")
    ...
    def update_file_metadata(self, filename, file_type, content, description=None, exports=None, imports=None):
        try:
            ...
            print_error(f"Error updating metadata for file {filename}: {str(e)}")