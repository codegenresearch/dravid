...
async def process_single_file(filename, content, project_context, folder_structure, metadata_manager):
    ...
    try:
        ...
        if summary_elem is None:
            print_warning(f"No summary provided for {filename}. Using default summary.")
            summary = "No summary available"

        print_success(f"Processed: {filename}")
        metadata_manager.update_file_metadata(filename, file_type, summary, exports, "")
...
async def process_files(files, project_context, folder_structure, metadata_manager):
    ...
    async def process_batch(batch):
        tasks = [process_single_file(filename, content, project_context, folder_structure, metadata_manager)
